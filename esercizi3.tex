...
\begin{enumerate}
\item Si ordinano le partizioni in base all'elemento più piccolo presente nel blocco.
\[
|1|2|3|
\]
\[
|1 3|2|
\]
(In questo caso restano uguali)
\item Se l'elemento $n$ ($n$ inteso come etichetta numerica associata al vertice) si trova in un blocco più grande nell'ordinamento (ordinamento definito nel punto 1), rispetto all'elemento $n-1$, si sposta l'elemento nel blocco inferiore.
\[
|12|3|-|
\]
\[
|123|-|
\]
\item Si eliminano i blocchi vuoti e si rinominano le etichette dei numeri, sottraendovi 1
\[
|01|2|
\]
\[
...
|12|
\]
Che effettivamente corrispondono alle nostre partizioni dell'insieme composto da $n-1$ elementi.
\end{enumerate}
Se si esegue il procedimento all'inverso, punto per punto, si ritorna alla partizione del path.
}
\newpage
\problemAnswer{
Volendo fare un esempio più complesso, prendiamo il seguente path con 4 vertici:
\begin{center}
\begin{tikzpicture}
...
\end{tikzpicture}
\end{center}
Il quale ha le seguenti 5 partizioni stabili:
\[
|1|2|3|4|
\]
\[
...
L'insieme corrispondente con 4-1=3 elementi $\{1,2,3\}$, ha sempre 5 partizioni:
\[
|1|2|3|
\]
\[
...
Partiamo sempre dalle partizioni stabili del cammino, ed eseguiamo l'ordinamento dei blocchi in base all'elemento più piccolo presente negli stessi \emph{(passo 1)}.
...
Poi spostiamo il vertice $n$ nel blocco precedente, se $n$ si trova in un blocco superiore a quello in cui è presente il vertice $n-1$ \emph{(passo 2)}:
\[
|12|3|4|-|
\]
\[
...
\problemAnswer{
Rinominiamo le etichette dei vertici facendo un -1 ed eliminiamo i blocchi vuoti \emph{(passo 3)}:
\[
|01|2|3|
\]
\[
...
Infine eliminiamo gli 0 \emph{(passo 4)}:
\[
|1|2|3|
\]
\[
...
|13|2|
\]
Che sono effettivamente le stesse partizioni dell'insieme composto da $n-1$ elementi. Eseguendo il passaggio all'inverso si può facilmente poi tornare alle partizioni del path.
}
