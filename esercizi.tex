...
\problemAnswer{
(i) Di base sappiamo che il numero di stirling di seconda specie S(n,k) si ottiene come
\[
S(n,k) = \frac{1}{k!}\sum_{i=0}^{k-1}(-1)^{i}\cdot {k \choose i} \cdot (k-i)^{n}
\]
Dato che stiamo considerando il caso in cui $k=2$ abbiamo che 
\[
S(n,2) = \frac{(1 \cdot 1 \cdot 2^{n})}{2!} + \frac{(-1 \cdot 2 \cdot 1^{n})}{2!} = 2^{n-1}-1
\]
\newline
...
\]
Che non è altro che la formula di Newton 
\[
(a+b)^n = \sum_{k=0}^{n}{n \choose k} a^{n-k}\cdot b^k
\]
in cui $a=b=1$ che porta ad avere
\[
(1+1)^n = 2^{n} = \sum_{k=0}^{n}{n \choose k} \cdot 1^{n-k} \cdot 1^{n} = \sum_{k=0}^{n}{n \choose k}
\]
}
\end{homeworkProblem}
...
\]
\problemAnswer{
Il numero di Stirling di prima specie indica il numero di cicli differenti ottenibili da un insieme di $n$ elementi distinti con $k$ cicli. Sapendo ciò possiamo affermare che per ottenere il numero di permutazioni con 1 solo ciclo, è necessario fissare uno degli elementi, e permturare gli altri. Come nell'esempio seguente, dove ho un insieme $A$ di cardinalità 4
\[
A = \{a, b, c, d\}
\] 
dove fisso l'elemento $a$ e permuto gli altri $n-1$ elementi
...
\[[a]dbc\]
\[[a]dcb\]
Che è uguale a $6$, e cioè $(4-1)!$. Quindi possiamo affermare che 
\[
s(n,1) = (n-1)!
\]
}
